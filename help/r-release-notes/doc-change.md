...
||[adobe.target.getOffers](/help/c-implementing-target/c-implementing-target-for-client-side-web/adobe-target-getOffers(options)-atjs-2.md#multiple)|Added the following section under adobe.target.getOffers():<ul><li>"Fetch and render data from multiple mboxes via getOffers() and applyOffers().</li></ul>|
